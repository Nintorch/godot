...
static EM_BOOL Emscripten_JoyStickConnected(int eventType, const EmscriptenGamepadEvent *gamepadEvent, void *userData)
{
    ...
    should_ignore = MAIN_THREAD_EM_ASM_INT({
        ...
        // DualSense support on Firefox is very bad (input lag, no dpad, wrong face buttons, no vibration),
    ...
    if (JoystickByIndex(gamepadEvent->index) != NULL || should_ignore) {
        goto done;
    }
